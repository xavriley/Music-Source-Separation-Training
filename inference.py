...
def run_folder(model, args, config, device, verbose=False):
    ...
    all_mixtures_path = [p for p in all_mixtures_path if p.split('.')[-1] in ["wav", "mp3", "flac"]]
    ...
    for path in all_mixtures_path:
        ...
        try:
            ...
            # mix = es.EasyLoader(filename=path, replayGain=3)()
            mix = es.EasyLoader(filename=path, replayGain=3)()
            sr = 44100 # default sr for essentia
            ...
            # mix = pyln.normalize.loudness(mix, loudness, -15.0)
            # only required when using sf to read

            # mix = mix.T # required for both methods
        ...
        if args.extract_instrumental:
            ...
            sf.write(instrum_file_name, mix - res['orch'].T, sr, subtype='FLOAT')
